...
func main() {
	...
	log.Info().Msg("single update")
	_, err = l.NewUpdateSession().Update(n)
	if err != nil {
		log.Panic().Err(err).Send()
	}
	log.Info().Msgf("%+v\n", *n)

	log.Info().Msg("update by slice")
	ss1 := []*Node{&Node{}, &Node{}}
	_, err = l.NewUpdateSession().Update(ss1)
	if err != nil {
		log.Panic().Err(err).Send()
	}
	log.Info().Msgf("%+v\n", ss1)

	log.Info().Msg("update by map")
	ss2 := map[string]*Node{
		"1": &Node{},
		"2": &Node{},
	}
	_, err = l.NewUpdateSession().Update(ss2)
	if err != nil {
		log.Panic().Err(err).Send()
	}
	log.Info().Msgf("%+v\n", ss2)
